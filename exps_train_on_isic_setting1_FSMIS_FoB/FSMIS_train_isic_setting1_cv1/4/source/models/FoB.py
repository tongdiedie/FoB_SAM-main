...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, supp_mask, qry_imgs, qry_labels, train):
        ...
        if supp_mask[[0], 0, 0].max() > 0. and qry_labels.max() > 0.:
            ...
            points_spt = self.uniform_sample_contour(supp_mask[0], num_keypoints=self.num_points) # [10,2]
    ...
    def uniform_sample_contour(self, mask, num_keypoints=10):
        ...
        Robust version: handles empty, tiny, single-point, or zero-length contours.
        """
        ...
        def fallback_points(binary_mask):
            ...
            coords = np.stack([xs, ys], axis=1).astype(np.float32)  # [x, y]

            if coords.shape[0] == 1:
                return np.repeat(coords, num_keypoints, axis=0).astype(int)

            idxs = np.linspace(0, coords.shape[0] - 1, num_keypoints).astype(int)
            return np.round(coords[idxs]).astype(int)

        ring = self.get_ring(mask, kernel_size=21)
        ...
        pts = contour[:, 0, :].astype(np.float32)  # [N, 2], x-y order
        ...
        sampled_points = np.round(np.asarray(sampled_points)).astype(int)
